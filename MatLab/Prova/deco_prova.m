...
function [] = deco_prova(N, EW, FW)
    ...
    elseif nargin < 1
        N = 20; % numero de vetores de teste aleatorios
    ...
    MSE = sum(erro)/N
    plot(erro)
    title('Comparação 64bits x 27bits');
    xlabel('Nmr. de Iterações') % x-axis label
    ylabel('Erro Quadrático Médio') % y-axis label
end
